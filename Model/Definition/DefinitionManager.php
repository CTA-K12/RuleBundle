...
class DefinitionManager implements DefinitionManagerInterface
{
    ...
    public function registerInput($name, $className, $params = []) {
        //Insert into the map
        $this->inputs[$name] = array('class' => $className, 'params' => $params);
    }
    ...
    public function registerContextAttribute($name, $contextName, $className, $inputName) {
        ...
        if (!array_key_exists($contextName, $this->contextAttributes)) {
            $this->contextAttributes[$contextName] = array();
        }

        $this->contextAttributes[$contextName][$name] = array('class' => $className, 'input' => $inputName);
    }
    ...
    public function registerContextAction($name, $contextName, $className, $inputName) {
        ...
        if (!array_key_exists($contextName, $this->contextActions)) {
            $this->contextActions[$contextName] = array();
        }

        $this->contextActions[$contextName][$name] = array('class' => $className, 'input' => $inputName);
    }
    ...
    public function registerServiceAttribute($name, $serviceName, $className, $inputName) {
        $this->serviceAttributes[$name] = array('service' => $serviceName, 'class' => $className, 'input' => $inputName);
    }
    ...
    public function registerServiceAction($name, $serviceName, $className, $inputName) {
        $this->serviceActions[$name] = array('service' => $serviceName, 'class' => $className, 'input' => $inputName);
    }
    ...
    public function getServiceAttribute($name) {
        ...
        //Get the attribute
        $attribute = $reflection->newInstanceArgs(array($tuple['service'], $this->container->get($tuple['service'])));
    ...
    public function getServiceAction($name) {
        ...
        $action = $reflection->newInstanceArgs(array($tuple['service'], $this->container->get($tuple['service'])));
